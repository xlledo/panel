...
@section('tools')
        <a href="{{ action('Ttt\Panel\GrupoController@index') }}" title="Volver al listado" class="btn btn-sm no-border"><i class="icon-double-angle-left"></i> Volver al listado</a>
	@if(Sentry::getUser()->hasAccess('grupos::crear') && $action != 'create')
		<a href="{{ action('Ttt\Panel\GrupoController@nuevo') }}" title="Nuevo elemento en {{ $_titulo }}" class="btn btn-sm btn-primary no-border"><i class="icon-file"></i> Nuevo</a></li>
        @endif
@stop
@section('page_header')
	@if($action == 'create')
	...
	@if(Sentry::getUser()->hasAccess('grupos::borrar'))
		@if ($action != 'create')
			@if($item->name != 'Superadmin')
				<div class="space-6"></div>
				<div class="acciones">
					<a class="btn btn-minier btn-danger no-border btn-confirmacion" title="Eliminar ?" href="{{ action('Ttt\Panel\GrupoController@borrar', $item->id) }}"><i class="icon-trash"></i>Borrar</a>
				</div>
			@endif
		@endif
	@endif

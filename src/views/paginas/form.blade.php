...
@section('tools')
        <a href="{{ action('Ttt\Panel\PaginasController@index') }}" title="Volver al listado" class="btn btn-sm no-border"><i class="icon-double-angle-left"></i> Volver al listado</a>
...
@section('content')
	...
	@if(Sentry::getUser()->hasAccess('paginas::borrar'))
		@if ($action != 'create')
			...
			<div class="acciones">
                                <a href="#" title="Borrar Página" class="btn btn-minier btn-danger no-border btn_confirmacion" data-action="{{action('Ttt\Panel\PaginasController@borrar', $item->id) }}"><i class="icon-trash"></i>Borrar Página</a>
			</div>
		@endif
	@endif
@stop

...
@stop
@section('page_header')
	<h1>Idiomas <small> <i class="icon-double-angle-right"></i> Listado</small></h1>
@stop
@section('content')
	<div class="row">
	    <div class="col-xs-12">
	        <div class="widget-box">
	            <form method="POST" action="{{ url('admin/idiomas') }}">
	                ...
	                <div class="widget-body collapse">
	                    ...
	                    <div class="widget-toolbox padding-8 clearfix">
	                        <div class="pull-right">
	                            <a href="{{ action('Ttt\Panel\IdiomaController@index') }}" title="Mostrar Todos" class="btn btn-primary btn-xs" >Mostrar todos</a></li>
	        ...
	        @else
	            ...
	            <form action="{{ url('admin/idiomas/acciones_por_lote') }}" method="post">
	                <fieldset>
	                    <table class="table table-striped table-bordered table-hover listado" summary="Listado de idiomas" border="0" cellpadding="0" cellspacing="1">
	                        <thead>
	                            <tr>
									...
									<th scope="col">{{ ordenable_link($currentUrl, 'codigo_iso_2', 'Código ISO', $params, $params[Config::get('panel::app.orderDir')]) }}</th>
									@if(Sentry::getUser()->hasAccess(array('idiomas::editar', 'idiomas::borrar'), FALSE))
	                        ...
	                        <tbody>
								@foreach($items as $index => $item)
									<tr class="@if($index % 2 == 0) par @else impar @endif">
										...
										<td class="td_click">
											@if(Sentry::getUser()->hasAccess('idiomas::editar'))
												{{ link_to('admin/idiomas/ver/' . $item->id, $item->nombre) }}
											@else
												{{ $item->nombre }}
											@endif
											@if($item->principal)
										...
										<td class="td_click">
											@if(Sentry::getUser()->hasAccess('idiomas::editar'))
												{{ link_to('admin/idiomas/ver/' . $item->id, $item->codigo_iso_2) }}
											@else
												{{ $item->codigo_iso_2 }}
											@endif
										</td>
...
@section('inline_js')
    ...
    $(".cambiar_estado").click(function(e){
        ...
        cambiar_estado($(this),'{{ url('admin/idiomas/cambiar_estado') }}');
    });
@stop

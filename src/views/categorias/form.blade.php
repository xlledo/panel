...
@section('tools')
    @if($action != 'createArbol' && Sentry::getUser()->hasAccess('categorias::verArbol'))
        <a href="{{ action('Ttt\Panel\CategoriaController@verArbol', $item->isRoot() ? $item->id : $item->getRoot()->id) }}" title="Volver al árbol {{ $item->isRoot() ? $item->nombre : $item->getRoot()->nombre }}" class="btn btn-sm no-border"><i class="icon-double-angle-left"></i> Volver al árbol</a>
    @endif
...
<div class="row">
    <div class="col-xs-12">
        <div id="tabs">
            ...
            <div id="datos">
                @if($item->isRoot())
                    <form class="clearfix" autocomplete="off" action="<?php echo ($action == 'createArbol') ? action('Ttt\Panel\CategoriaController@crearArbol') : action('Ttt\Panel\CategoriaController@actualizarRaiz'); ?>" method="post">
                @else
                    <form class="clearfix" autocomplete="off" action="<?php echo ($action == 'create') ? action('Ttt\Panel\CategoriaController@crear') : action('Ttt\Panel\CategoriaController@actualizar'); ?>" method="post">
                @endif
                                                    ...
                                                    <div class="form-group @if ($errors->first('slug')) has-error @endif">
                                                        ...
                                                        @if(Sentry::getUser()->isSuperUser())
                                                            @if($item->isRoot())
                                                                ...
                                                                @else
                                                                    <input type="text" tabIndex="4" class="form-control" name="slug" id="slug" value="{{ $item->slug }}" size="20" />
                                                                @endif
                                                            @else
                                                                @if($item->getRoot()->protegida)
                                                                    <input type="text" tabIndex="4" class="form-control" name="slug" id="slug" value="{{ $item->slug }}" readonly="readonly" size="20" />
                                                                @else
                                                                    <input type="text" tabIndex="4" class="form-control" name="slug" id="slug" value="{{ $item->slug }}" size="20" />
                                                                @endif

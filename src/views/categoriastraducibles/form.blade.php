...
@section('page_header')
	@if($action == 'create')
		<h1>Nueva subcategoría en {{ link_to('admin/categorias-traducibles/ver-arbol/' . $item->getRoot()->id, $item->getRoot()->nombre, array('title' => $item->getRoot()->nombre)) }}</h1>
	@elseif($action == 'createArbol')
		<h1>Nuevo árbol de <a href="{{ action('Ttt\Panel\CategoriaTraducibleController@index') }}" title="Volver al listado">Taxonomías traducibles</a></h1>
	@else

...
@section('tools')
	@if(Sentry::getUser()->hasAccess('categorias-traducibles::crear-arbol'))
		<a href="{{ action('Ttt\Panel\CategoriaTraducibleController@nuevoArbol') }}" title="Nuevo árbol de {{ $_titulo }}" class="btn btn-sm btn-primary no-border"><i class="icon-file"></i> Nuevo árbol de {{ $_titulo }}</a></li>
	@endif
@stop
@section('page_header')
